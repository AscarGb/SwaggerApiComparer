...
namespace ApiComparer.Lib
{
    public class Net48ApiParser
    {
        ...
        {
            foreach (var definition in _oldApiObject["definitions"].Children())
            {
        ...
        private void ParseResponse(JToken response, List<ResponseStatusCode> responses)
        {
            ...
            var schema = response.Children()["schema"];

            if (schema["type"].Any())
            {
                var shemaType = (string)schema["type"].FirstOrDefault();

                if (shemaType.Equals("array", StringComparison.OrdinalIgnoreCase))
                {
                    isArray = true;
                    key = (string)schema["items"]["$ref"].FirstOrDefault();
                }
                else
                    key = shemaType;
            }
            ...
            if (modelType != null)
            {
                var modelTypeObject = _oldApiObject["definitions"]?[modelType];
